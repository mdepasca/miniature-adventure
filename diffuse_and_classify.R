...
diffuse.and.classify <- function(dmap.eps.val, distance.matrix, training.set, test.set){
    ...
    sn.rf <- randomForest(x=dmapCoord.train[,1:(ncols-1)], y=as.factor(dmapCoord.train[,ncols]), xtest=dmapCoord.test[,1:(ncols-1)], ytest=as.factor(dmapCoord.test[,ncols]), importance=TRUE)
